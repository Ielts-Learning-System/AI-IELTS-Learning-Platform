...
const convertRawToBand = (rawScore, moduleType = 'listening') => {
  const normalizedRawScore = normalizeScore(rawScore);
  const normalizedModule = String(moduleType || '').toLowerCase();

  // Listening and Reading Academic share this baseline conversion in this service.
  if (normalizedModule !== 'listening' && normalizedModule !== 'reading') {
    return 0;
  }
