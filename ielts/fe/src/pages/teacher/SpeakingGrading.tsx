...
const getStudentLabel = (submission: SpeakingSubmission) => {
...
export default function SpeakingGrading() {
  ...
  const [pendingSubmissions, setPendingSubmissions] = useState<SpeakingSubmission[]>([]);
  ...
  const fetchPendingSubmissions = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch pending speaking submissions:', error);
      toast.error(error.response?.data?.message || 'Không thể tải danh sách Speaking chờ chấm.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmitGrade = async () => {
    if (!selectedSubmission) {
      toast.error('Vui lòng chọn bài nói cần chấm.');
      return;
    }
    ...
    try {
      ...
      await axios.put(
        `${API_BASE}/api/speaking/${selectedSubmission._id}/grade`,
        {
          criteria: {
            FC: gradeForm.FC,
            LR: gradeForm.LR,
      ...
      console.error('Failed to submit speaking grade:', error);
      ...
      <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-[linear-gradient(135deg,#fef2f2_0%,#ffffff_42%,#f8fafc_100%)] shadow-[0_24px_80px_rgba(15,23,42,0.10)]">
        <div className="border-b border-red-100 bg-white/85 px-6 py-6 backdrop-blur sm:px-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-2">
              ...
              <h2 className="text-3xl font-black tracking-tight text-slate-900">Teacher Speaking Workspace</h2>
              <p className="max-w-2xl text-sm leading-6 text-slate-500">
                Chọn bài nói trong danh sách chờ, nghe audio trực tiếp và chấm 4 tiêu chí IELTS Speaking trong một màn hình split-pane.
              </p>
            </div>
        ...
        >
          <aside className="border-r border-red-100 bg-[linear-gradient(180deg,#ffffff_0%,#fff7f7_100%)] p-4 sm:p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Pending List</p>
            <h3 className="mt-1 text-lg font-bold text-slate-900">Bài nói chờ chấm</h3>

            <div className="mt-4 space-y-3">
              {isLoading ? (
                <div className="flex min-h-[240px] items-center justify-center rounded-[24px] border border-red-50 bg-white">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                            <span className="rounded-full bg-white px-2.5 py-1 ring-1 ring-red-100">
            ...
            {!selectedSubmission ? (
              <div className="flex min-h-[640px] items-center justify-center rounded-[30px] border border-dashed border-red-200 bg-white text-center">
                <div className="max-w-md px-6">
                  <Volume2 className="mx-auto mb-4 h-12 w-12 text-red-400" />
                  <p className="text-xl font-bold text-slate-900">Chọn một bài nói để bắt đầu chấm</p>
                </div>
              </div>
            ) : (
              <Split
                ...
                <div className="border-r border-red-100 bg-[linear-gradient(180deg,#ffffff_0%,#fff8f8_100%)] p-6">
                  <div className="space-y-6">
                    <div className="rounded-[26px] border border-red-100 bg-white p-5 shadow-sm">
                      ...
                      <h3 className="mt-1 text-xl font-black text-slate-900">{getStudentLabel(selectedSubmission)}</h3>
                  ...
                  <div className="flex h-full flex-col gap-5">
                          ...
                          {criteriaConfig.map((criterion) => (
                            <div key={criterion.key} className="rounded-[22px] border border-red-50 bg-[linear-gradient(180deg,#fffafa_0%,#ffffff_100%)] p-4">
                              <div className="mb-3 flex items-start justify-between gap-4">
                                <div>
                                  <p className="font-bold text-slate-900">{criterion.key} · {criterion.label}</p>
                              ...
                              <input
                                ...
                                onChange={(event) => handleCriteriaChange(criterion.key, Number(event.target.value))}
                    ...
                    <div className="flex flex-1 flex-col rounded-[26px] border border-red-100 bg-white p-5 shadow-sm">
                      ...
                      <textarea
                        ...
                        className="min-h-[240px] flex-1 resize-none rounded-[22px] border border-red-50 bg-[linear-gradient(180deg,#fffafa_0%,#ffffff_100%)] px-5 py-4 text-[15px] leading-7 text-slate-700 outline-none transition placeholder:text-slate-300 focus:border-red-200 focus:ring-4 focus:ring-red-100"
                      />
